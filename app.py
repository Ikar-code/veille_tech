    ...
    with col_c:
            ...
            if st.button("Se connecter", use_container_width=True, type="primary", key="btn_login"):
                ...
                if not email_val or not pwd_val:
                    ...
                    with st.spinner("Connexion…"):
                        res = auth.connecter(email_val, pwd_val)
                    if res["ok"]:
                        st.session_state.update({
                            "user":    res["user"],
                            "session": res["session"],
                            "profil":  auth.get_profil(res["user"].id),
                            "page":    "veille",
                            "chat_history_loaded": False,
                        })
        ...
        with tab_in:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            st.text_input("Email", key="reg_email", placeholder="votre@email.com")
            st.text_input("Mot de passe", type="password", key="reg_pwd", help="Minimum 6 caractères")
            st.text_input("Confirmer", type="password", key="reg_pwd2")
