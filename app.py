# ============================================================
# APP.PY — Veille IA — isolation par utilisateur + auto
# ============================================================
    ...
    with st.sidebar:
        ...
        if not abonne:
            st.markdown(
                ...
                '</div>',unsafe_allow_html=True)
            if st.button("✨ S'abonner",use_container_width=True,key="btn_sub"):
...
def page_auto():
    ...
    st.markdown(
        ...
        'L\'heure est en UTC. La Réunion = UTC+4, France métropolitaine = UTC+1 ou UTC+2 (été).'
        '</div>',unsafe_allow_html=True)
    ...
    with col2:
        minute_utc=st.selectbox(
            "Minute",
            [0,15,30,45],
            index=[0,15,30,45].index(int(prefs.get("minute",0))) if int(prefs.get("minute",0)) in [0,15,30,45] else 0,
            format_func=lambda m: f"{m:02d}"
        )
    with col3:
        # Convertit en heure locale Réunion pour l'affichage
    ...
    actif_val=prefs.get("actif",True)
    actif=st.toggle("Activer la veille automatique",value=bool(actif_val))
    ...
    if prefs.get("derniere_execution"):
        try:
            ...
            d_local=(d.replace(tzinfo=tz.utc) if d.tzinfo is None else d)
            st.markdown(
                f'<div style="font-size:12px;color:var(--subtext);margin-top:8px;">'
                f'Dernière exécution : {d_local.strftime("%d/%m/%Y à %H:%M")} UTC</div>',
                unsafe_allow_html=True)
        except Exception:
            pass
    ...
    if st.button("💾 Enregistrer la configuration",type="primary",use_container_width=False):
        if not sujets.strip():
            ...
            if ok:
                if actif:
                    st.success(f"✅ Veille automatique activée — envoi chaque jour à {h_reunion:02d}h{minute_utc:02d} (heure Réunion)")
                else:
                    st.info("Veille automatique désactivée.")
            else:
                st.error("Erreur lors de l'enregistrement. Vérifiez la connexion Supabase.")
        else:
    ...
    st.markdown(
        ...
        '<div style="font-size:13px;font-weight:600;color:var(--text);margin-bottom:8px;">Comment ça fonctionne</div>'
        ...
        '2. À l\'heure que vous choisissez, il lance la recherche sur vos sujets<br>'
        '3. Il génère les résumés IA et vous envoie un email récapitulatif<br>'
        '4. Vous n\'avez rien à faire — tout se passe sans que vous soyez connecté'
        '</div></div>',unsafe_allow_html=True)
