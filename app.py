...
try:
    ...
    print(f"[app] module chatbot indisponible: {e}")
...
/* ── Chat ── */
.chat-bubble-user{background:var(--blue);color:var(--bg);border-radius:16px 16px 4px 16px;padding:10px 14px;font-size:13px;max-width:80%;margin-left:auto;margin-bottom:8px;line-height:1.6;}
.chat-bubble-bot{background:var(--overlay);color:var(--text);border-radius:16px 16px 16px 4px;padding:10px 14px;font-size:13px;max-width:80%;margin-right:auto;margin-bottom:8px;line-height:1.6;border-left:3px solid var(--blue);}
.chat-wrap{display:flex;flex-direction:column;padding:16px;}
.chat-meta{font-size:10px;color:var(--subtext);margin-top:2px;margin-bottom:4px;}
.chat-meta-right{text-align:right;}
.chat-container{background:var(--surface);border:1px solid var(--border);border-radius:12px;max-height:460px;overflow-y:auto;padding:8px;}
/* ── Animations ── */
...
INTERVALLES_INV = {v: k for k, v in INTERVALLES.items()}
...
def _fusionner_historique_import(h_existant, h_import):
    out = dict(h_existant)
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        if user:
            ...
            if not abonne:
                st.markdown(
                    '<div style="background:rgba(203,166,247,.08);border:1px solid var(--mauve);border-radius:10px;padding:10px;margin-bottom:12px;text-align:center;">'
            ...
            if st.button("🚪 Déconnexion", use_container_width=True, key="sb_logout"):
                ...
                st.session_state.update({"user":None,"session":None,"profil":{},
                    "page":"accueil","chat_messages":[],"chat_charge":False,
...
def page_accueil():
    if not AUTH_OK:
        st.error("Service d'authentification indisponible."); return
    ...
    with col_c:
        ...
        with tab_co:
            ...
            if st.button("Se connecter", use_container_width=True, type="primary", key="btn_login"):
                if not email or not pwd: st.error("Remplissez tous les champs.")
                else:
                    ...
                    if res["ok"]:
                        st.session_state.update({"user":res["user"],"session":res["session"],
                            ...
                            "chat_messages":[],"chat_charge":False})
            ...
            if st.button("Mot de passe oublié ?", use_container_width=True, key="btn_reset_pwd"):
                ...
                else: st.warning("Entrez votre email d'abord.")
            st.markdown("---")
...
# ============================================================
# PAGE CHAT — Support avec mémoire persistante
# ============================================================
def page_chat():
    st.markdown("# 💬 Support")
    st.markdown("### Posez vos questions sur la plateforme Veille IA")
    st.markdown("---")

    uid = _user_id()

    if not CHATBOT_OK:
        st.error("Module chatbot indisponible. Vérifiez que chatbot.py est présent.")
        return

    # ── Chargement de la mémoire depuis Supabase (une seule fois par session) ──
    if not st.session_state.get("chat_charge") and uid and STORAGE_OK:
        try:
            historique_db = storage.charger_historique_chat(uid, limite=40)
            st.session_state["chat_messages"] = historique_db
            st.session_state["chat_charge"]   = True
            if historique_db:
                nb = len(historique_db)
                st.toast(f"💬 {nb} message(s) de conversation chargés", icon="📂")
        except Exception as e:
            print(f"[chat] Erreur chargement mémoire : {e}")
            st.session_state["chat_charge"] = True

    col_chat, col_aide = st.columns([2, 1], gap="large")

    with col_chat:
        # ── Infos mémoire ──────────────────────────────────
        nb_msgs = len(st.session_state["chat_messages"])
        if nb_msgs > 0:
            st.markdown(
                f'<div style="font-size:12px;color:var(--subtext);margin-bottom:12px;display:flex;align-items:center;gap:10px;">'
                f'<span>🧠 Mémoire active — {nb_msgs} message(s) en mémoire</span>'
        ...
        if not st.session_state["chat_messages"]:
            st.markdown(
                '<div style="text-align:center;padding:40px 20px;color:var(--subtext);">'
                '<div style="font-size:32px;margin-bottom:12px;">🤖</div>'
                '<div style="font-size:14px;font-weight:600;margin-bottom:6px;">Assistant Veille IA</div>'
                '<div style="font-size:12px;">Posez vos questions ou cliquez sur une suggestion</div>'
                '</div>', unsafe_allow_html=True)
        else:
            ...
            for msg in st.session_state["chat_messages"]:
                ...
                if role == "user":
                    st.markdown(
                        f'<div class="chat-meta chat-meta-right">Vous</div>'
                        f'<div class="chat-bubble-user">{content}</div>',
                        unsafe_allow_html=True)
                elif role == "assistant":
                    st.markdown(
                        f'<div class="chat-meta">🤖 Assistant</div>'
        ...
        with col_btn:
            btn_envoyer = st.button("📨", use_container_width=True, key="btn_chat_envoyer",
                                     help="Envoyer")

        # ── Envoi du message ───────────────────────────────
        if btn_envoyer and message_input.strip():
            msg_u = message_input.strip()
            ok_val, _ = _valider_texte(msg_u, 1000)
            if not ok_val:
                st.warning("Message trop long (max 1000 caractères).")
            else:
                with st.spinner("Réponse en cours…"):
                    try:
                        if uid and STORAGE_OK:
                            # Utilise la mémoire persistante Supabase
                            reponse = chatbot.repondre_avec_memoire(
                                uid, msg_u,
                                storage_module=storage)
                        else:
                            # Fallback sans persistance
                            hist_tmp = st.session_state["chat_messages"].copy()
                            hist_tmp.append({"role":"user","content":msg_u})
                            reponse = chatbot.repondre(hist_tmp)

                        # Met à jour l'affichage local
                        st.session_state["chat_messages"].append({"role":"user",     "content":msg_u})
        ...
        if nb_msgs > 0:
            ...
            with col_eff2:
                if st.button("🗑 Effacer", use_container_width=True, key="btn_clear_chat",
                             help="Efface toute la mémoire de conversation"):
                    if uid and STORAGE_OK:
                        try: storage.effacer_historique_chat(uid)
                        except Exception: pass
    ...
    with col_aide:
        st.markdown(
            '<div style="font-family:Space Mono;font-size:12px;color:var(--blue);margin-bottom:12px;">Questions fréquentes</div>',
            unsafe_allow_html=True)

        questions = getattr(chatbot, "QUESTIONS_PREDEFINIES",
                            ["Comment lancer une veille ?","Comment configurer le FTP ?",
                             "C'est quoi l'abonnement ?","Pourquoi ça reste bloqué ?"])

        for q in questions:
            if st.button(q, use_container_width=True, key=f"chat_q_{q[:20]}"):
                with st.spinner("Réponse en cours…"):
                    try:
                        if uid and STORAGE_OK:
                            reponse = chatbot.repondre_avec_memoire(uid, q, storage_module=storage)
                        else:
                            hist_tmp = st.session_state["chat_messages"].copy()
                            hist_tmp.append({"role":"user","content":q})
                            reponse = chatbot.repondre(hist_tmp)
        ...
        st.markdown(
            ...
            '<div style="font-size:12px;font-weight:600;margin-bottom:6px;">🧠 Mémoire persistante</div>'
            '<div style="font-size:11px;color:var(--subtext);line-height:1.7;">'
            'Vos conversations sont sauvegardées entre les sessions. '
            'L\'assistant se souvient de vos échanges précédents.</div>'
            '</div>', unsafe_allow_html=True)

        st.markdown(
            '<div class="card" style="padding:12px 14px;margin-top:8px;">'
            '<div style="font-size:12px;font-weight:600;margin-bottom:6px;">📧 Contact direct</div>'
            '<div style="font-size:11px;color:var(--subtext);">lucas.rajanysio@gmail.com</div>'
            '</div>', unsafe_allow_html=True)

# ============================================================
# PAGE AUTOMATISATION — avec intervalle
# ============================================================
def page_auto():
    st.markdown("# ⏰ Veille automatique")
    st.markdown("### Recevez votre veille par email à la fréquence de votre choix")
    st.markdown("---")
    ...
    st.markdown("#### 🔄 Fréquence de la veille")
    intervalle_actuel = int(prefs.get("intervalle_jours") or 1)
    label_actuel      = INTERVALLES_INV.get(intervalle_actuel, "Chaque jour")
    opts_labels       = list(INTERVALLES.keys())
    idx_actuel        = opts_labels.index(label_actuel) if label_actuel in opts_labels else 0

    col_freq, col_info = st.columns([2, 1])
    with col_freq:
        intervalle_label = st.selectbox(
            "Envoyer la veille…",
            opts_labels,
            index=idx_actuel,
            key="sel_intervalle",
            help="Définit le nombre de jours minimum entre deux envois")
        intervalle_jours = INTERVALLES[intervalle_label]
    with col_info:
        # Calcul de la prochaine exécution estimée
        derniere = prefs.get("derniere_execution")
        if derniere:
            try:
                dt_d = datetime.fromisoformat(str(derniere).replace("Z",""))
                from datetime import timedelta
                prochaine = dt_d + timedelta(days=intervalle_jours)
                st.markdown(
                    f'<div class="metric-box" style="margin-top:20px;">'
                    f'<span class="metric-val" style="font-size:16px;">{prochaine.strftime("%d/%m")}</span>'
                    f'<span class="metric-lbl">Prochaine veille estimée</span></div>',
                    unsafe_allow_html=True)
            except Exception:
                pass
        else:
            st.markdown(
                f'<div class="metric-box" style="margin-top:20px;">'
                f'<span class="metric-val" style="font-size:14px;">{intervalle_label}</span>'
                f'<span class="metric-lbl">Fréquence choisie</span></div>',
                unsafe_allow_html=True)
    ...
    with col2:
        opts_min = [0,15,30,45]; min_val = int(prefs.get("minute",0))
        minute_utc = st.selectbox("Minute", opts_min,
                                   index=opts_min.index(min_val) if min_val in opts_min else 0,
    ...
    with col3:
        ...
        st.markdown(
            ...
            f'<span class="metric-lbl">Heure Réunion (UTC+4)</span></div>',
            unsafe_allow_html=True)
    ...
    if prefs.get("derniere_execution"):
        try:
            d = datetime.fromisoformat(str(prefs["derniere_execution"]).replace("Z","+00:00").replace("+00:00",""))
            st.markdown(
    ...
    if st.button("💾 Enregistrer", type="primary"):
        ...
        if not ok_val:
            ...
            try:
                ok = storage.sauvegarder_veille_auto(
                    sujets.strip(), int(heure_utc), int(minute_utc),
                    actif, uid, intervalle_jours)
                if ok:
                    st.success(
                        f"✅ Veille {'activée' if actif else 'désactivée'} — "
                        f"{intervalle_label} à {h_reunion:02d}h{minute_utc:02d} (Réunion)")
                else:
    ...
    st.markdown(
        ...
        '2. À l\'heure choisie, il vérifie si l\'intervalle choisi est écoulé<br>'
        '3. Si oui, il lance la recherche sur vos sujets et génère les résumés IA<br>'
        '4. Il vous envoie un email de synthèse et publie sur FTP si configuré<br>'
        '5. Vous n\'avez rien à faire — tout se passe sans connexion'
...
def _render_theme_editor():
    ...
    st.markdown('<div style="font-family:Space Mono;font-size:13px;color:var(--blue);">🎨 Personnaliser le thème de veille-ia.html</div>', unsafe_allow_html=True)
    st.markdown('<div style="font-size:12px;color:var(--subtext);margin:6px 0 16px 0;">Modifiez l\'apparence de la page publiée sur votre hébergement FTP.</div>', unsafe_allow_html=True)
    PRESETS = {
    ...
    for i,(label,vals) in enumerate(PRESETS.items()):
        with cols[i]:
            if st.button(label, use_container_width=True, key=f"preset_{i}"): _appliquer_theme(vals); st.rerun()
    with cols[len(PRESETS)]:
        if st.button("🔄 Reset", use_container_width=True, key="btn_reset_theme"):
            _appliquer_theme(dict(THEME_DEFAULT))
            try: cfg=_cfg(); cfg.pop("theme_ftp",None); _save_cfg(cfg)
            except Exception: pass
            st.rerun()
    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    ...
    with col_left:
        ...
        sc = st.columns(2)
        for i,(key,label) in enumerate([("bg","Fond page"),("surf","Fond carte"),("ov","Fond hover"),("txt","Texte"),("sub","Sous-texte"),("brd","Bordure"),("blue","Bleu (liens)"),("grn","Vert"),("yel","Jaune (synthèse)"),("red","Rouge")]):
            with sc[i%2]:
    ...
    with col_right:
        ...
        font = st.selectbox("Police", font_opts, index=font_opts.index(cur_font) if cur_font in font_opts else 0, key=f"th_font_v{ver}")
    ...
    st.markdown(
        ...
        f'📅 {datetime.now().strftime("%d/%m/%Y")} — 5 articles <span style="background:#a6e3a1;color:#1e1e2e;font-size:10px;padding:1px 6px;border-radius:6px;font-weight:700;">NOUVEAU</span></div>'
        f'</div>'
        ...
        f'— Synthèse · La convergence des modèles open-source s\'accélère [1]</div></div>',
        unsafe_allow_html=True)
    ...
    with col_apply:
        if st.button("🎨 Appliquer & republier sur FTP", type="primary", use_container_width=True, key="btn_apply_theme"):
            ...
            else: st.warning("FTP non configuré — allez dans Configuration.")
    with col_save:
...
def page_abonnement():
    ...
    if _est_abonne():
        st.markdown('<div class="card card-green" style="padding:24px;text-align:center;"><div style="font-size:32px;margin-bottom:8px;">✅</div><div style="font-size:18px;font-weight:600;color:var(--green);margin-bottom:8px;">Abonnement actif</div><div style="font-size:13px;color:var(--subtext);">Recherches illimitées + veille email automatique.</div></div>', unsafe_allow_html=True)
        ...
        with col_c:
            ...
            st.markdown(
                ...
                '✔ Intervalle personnalisé (quotidien, hebdo, mensuel…)<br>'
...
def page_veille():
    ...
    uid=_user_id(); abonne=_est_abonne()
    if AUTH_OK and uid:
        if not abonne:
            quota = auth.get_quota(uid)
            reste = max(0, auth.RECHERCHES_GRATUITES - quota.get("searches_used",0))
            if reste == 0:
                st.markdown('<div class="card card-red" style="padding:24px;text-align:center;"><div style="font-size:20px;margin-bottom:8px;">🔒</div><div style="font-size:16px;font-weight:600;color:var(--red);margin-bottom:8px;">Limite gratuite atteinte</div><div style="font-size:13px;color:var(--subtext);">Abonnez-vous pour un accès illimité.</div></div>', unsafe_allow_html=True)
                if st.button("✨ S'abonner à 2,99€/mois", type="primary"): _goto("abonnement")
                return
    ...
    with col_form:
        ...
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        c1,c2 = st.columns(2)
        with c1: btn_rechercher = st.button("🔍 Rechercher", use_container_width=True, disabled=st.session_state["en_cours"], type="primary", key="btn_rechercher")
        with c2:
            if st.button("🗑 Réinitialiser", use_container_width=True, key="btn_reset_veille"):
                st.session_state.update({"logs":[],"resultats":[],"recherche_terminee":False,"derniere_publication":None}); st.rerun()
        if st.session_state["resultats"] and not st.session_state["en_cours"]:
            ...
            for r in st.session_state["resultats"][:8]:
                dom=urlparse(r.get("href","")).netloc; score=r.get("score",0)
                c="green" if score>=80 else "yellow" if score>=50 else "red"
                st.markdown(f'<div class="card" style="padding:10px 14px;margin-bottom:6px;"><div style="font-size:13px;font-weight:500;margin-bottom:3px;">{r.get("title","")[:72]}…</div><div style="display:flex;gap:8px;"><span style="font-size:11px;color:var(--subtext)">{dom}</span>{_badge(f"score {score}",c)}</div></div>', unsafe_allow_html=True)
            if len(st.session_state["resultats"])>8: st.caption(f"… et {len(st.session_state['resultats'])-8} autres articles")
    with col_log:
        if st.session_state["en_cours"]:
            sa=st.session_state.get("sujet_courant","…"); dl=st.session_state.get("dernier_log","Initialisation…")
            st.markdown(f"""<div class="launch-overlay"><div class="scan-line"></div><div class="launch-icon-wrap"><span class="launch-icon">🔭</span><div class="pulse-ring"></div><div class="pulse-ring"></div><div class="pulse-ring"></div></div><div class="launch-title">RECHERCHE EN COURS</div><div class="launch-subject">« {sa} »</div><div class="launch-steps"><div class="launch-step"><span class="step-icon">🔍</span><span class="step-label">DuckDuckGo</span></div><div class="launch-step"><span class="step-icon">📡</span><span class="step-label">Flux RSS</span></div><div class="launch-step"><span class="step-icon">⚙️</span><span class="step-label">Scoring</span></div><div class="launch-step"><span class="step-icon">🧹</span><span class="step-label">Dédup.</span></div></div><div class="typing-dots"><span></span><span></span><span></span></div><div class="launch-status">{dl}</div></div>""", unsafe_allow_html=True)
        else:
    ...
    if btn_rechercher and sujet.strip() and not st.session_state["en_cours"]:
        ...
        if not ok_val: st.error(msg_val); return
        if AUTH_OK and uid:
            ok_q,msg_q = auth.peut_rechercher(uid)
            if not ok_q: st.error(msg_q); return
        st.session_state.update({"en_cours":True,"resultats":[],"sujet_courant":sujet.strip(),"limite_courante":int(limite),"logs":[],"dernier_log":"Initialisation…","recherche_terminee":False,"derniere_publication":None})
        st.rerun()
        ...
        except Exception as e: _log(f"❌ Erreur : {e}")
        st.session_state.update({"en_cours":False,"dernier_log":"","recherche_terminee":True}); st.rerun()
    elif btn_rechercher and not sujet.strip(): st.warning("Entrez au moins un sujet.")
...
def _render_panneau_publication(uid, abonne):
    nb_r=len(st.session_state["resultats"]); limite=st.session_state.get("limite_courante",10); sujet=st.session_state.get("sujet_courant","")
    st.markdown("---")
    ...
    st.markdown(f'<div style="font-size:12px;color:var(--subtext);margin-bottom:16px;">{nb_r} articles trouvés pour <strong style="color:var(--text)">{sujet}</strong>.</div>', unsafe_allow_html=True)
    col_p1,col_p2 = st.columns(2)
    with col_p1: nb_articles = st.number_input("Articles à résumer et publier", min_value=1, max_value=min(nb_r,50), value=min(limite,nb_r), step=1, key="pub_nb_articles")
    with col_p2: mode_pub = st.selectbox("Mode WordPress", ["Mise à jour page","Créer un post"], key="pub_mode")
    st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
    ...
    if pub:
        ...
        for col,ok_key,msg_key,label in [(col_r1,"ok_wp","msg_wp","WordPress"),(col_r2,"ok_ftp","msg_ftp","FTP / Page web")]:
            with col:
                ok=pub.get(ok_key); msg=pub.get(msg_key,"")
                if ok is not None:
    ...
    cible_wp=btn_wp or btn_both; cible_ftp=btn_ftp or btn_both
    if (cible_wp or cible_ftp) and not st.session_state.get("en_cours_pub",False):
        ...
        with st.spinner("Génération des résumés IA et publication…"):
            try:
                if mode_pub=="Mise à jour page" or cible_ftp:
                    res=srv.workflow_publier(sujet,st.session_state["resultats"],callback_statut=_log,limite=int(nb_articles),theme_ftp=st.session_state.get("theme_ftp"),publier_wp=bool(cible_wp),publier_ftp_flag=bool(cible_ftp))
        ...
        st.session_state["derniere_publication"]=pub_result; st.session_state["en_cours_pub"]=False; st.rerun()
...
def page_historique():
    ...
    h=_historique(); sujets=[k for k in h if not k.startswith("__") and isinstance(h[k],list)]
    with st.expander("📥 Importer depuis un fichier veille-ia.html", expanded=not sujets):
        ...
        if fichier is not None:
            try: contenu_html=fichier.read().decode("utf-8",errors="replace")
            except Exception as e: st.error(f"Lecture impossible : {e}")
            else:
                hist_imp=srv.extraire_historique_depuis_html(contenu_html)
                if hist_imp:
                    ns=len([k for k in hist_imp if not k.startswith("__")]); na=sum(len(s.get("articles",[])) for v in hist_imp.values() if isinstance(v,list) for s in v if isinstance(s,dict))
                    ...
                    if st.button("Fusionner avec mon historique", type="primary", key="btn_merge_hist"):
                        merged=_fusionner_historique_import(h,hist_imp); _sauvegarder_historique_complet(merged)
                        st.success("Historique mis à jour."); st.rerun()
                else: st.warning("Aucune donnée reconnue dans ce fichier.")
    if not sujets: st.info("Aucun historique. Lancez une première veille ou importez un fichier HTML."); return
    st.markdown("---")
    col_g,col_d=st.columns([1,2],gap="large")
    with col_g:
        sujet_sel=st.selectbox("Sujet à afficher",sujets,key="hist_sujet_sel"); sessions=h.get(sujet_sel,[])
        ...
        st.markdown("---"); st.markdown("#### Supprimer des sujets")
    ...
    with col_d:
        for i,session in enumerate(sessions):
            ...
            date_s=session.get("date","?"); articles=session.get("articles",[]); rg=session.get("resume_global","")
            with st.expander(f"📅 {date_s} — {len(articles)} articles",expanded=(i==0)):
                ...
                for a in sorted(articles,key=lambda x:x.get("score",0),reverse=True):
                    dom=urlparse(a.get("href","")).netloc; score=a.get("score",0); points=a.get("resume_ollama",[])
                    pts_html=""
                    ...
                    st.markdown(f'<div class="card" style="padding:12px 16px;margin-bottom:6px;"><div style="display:flex;justify-content:space-between;"><a href="{a.get("href","")}" target="_blank" style="color:var(--blue);font-size:13px;font-weight:500">{a.get("title","")[:80]}</a><span class="badge badge-blue">{score}</span></div><div style="font-size:11px;color:var(--subtext);margin-top:4px">{dom}</div>{pts_html}</div>', unsafe_allow_html=True)
...
def page_comparaison():
    ...
    sujet_sel=st.selectbox("Sujet",sujets); sessions=[s for s in h.get(sujet_sel,[]) if isinstance(s,dict)]
    ...
    sess_rec=next((s for s in sessions if s.get("date")==date_rec),None); sess_anc=next((s for s in sessions if s.get("date")==date_anc),None)
    if not sess_rec or not sess_anc: st.error("Sessions introuvables."); return
    hrefs_anc={a["href"] for a in sess_anc.get("articles",[]) if "href" in a}; hrefs_rec={a["href"] for a in sess_rec.get("articles",[]) if "href" in a}
    ...
    for col,val,lbl,clr in [(c1,len(sess_rec.get("articles",[])),"Récents","blue"),(c2,len(nouveaux),"Nouveaux","green"),(c3,len(disparus),"Disparus","red")]:
        with col: st.markdown(f'<div class="metric-box"><span class="metric-val" style="color:var(--{clr})">{val}</span><span class="metric-lbl">{lbl}</span></div>', unsafe_allow_html=True)
...
def page_config():
    ...
    cfg=_cfg(); ftp_host=cfg.get("ftp_host",""); exemple_url=f"https://{ftp_host}/veille-ia.html" if ftp_host else "https://monsite.com/veille-ia.html"
    tab_wp,tab_ftp,tab_theme,tab_integration=st.tabs(["🌐 WordPress","📡 FTP","🎨 Thème & Affichage","🔗 Intégration"])
    with tab_wp:
        ...
        with ct:
            if st.button("🔌 Tester WP",use_container_width=True,key="btn_test_wp"):
                ok,msg=srv.tester_connexion_wp(wp_base,wp_user,wp_pwd); st.success(msg) if ok else st.error(msg)
    with tab_ftp:
        ...
        with ct:
            if st.button("🔌 Tester FTP",use_container_width=True,key="btn_test_ftp"):
                ok,msg=srv.tester_connexion_ftp(ftp_host_input,ftp_user,ftp_pwd); st.success(msg) if ok else st.error(msg)
    with tab_theme: _render_theme_editor()
    with tab_integration:
        st.markdown("#### 🔗 Intégrer la veille sur une autre page")
        url_veille=st.text_input("URL publique de votre veille-ia.html",value=exemple_url,key="integration_url")
        st.markdown("---"); st.markdown("#### ⚙️ Options d'affichage")
        col_pos,col_larg=st.columns(2)
        with col_pos: position=st.selectbox("Position",["Centré (milieu)","Gauche","Droite","Pleine largeur"],key="iframe_position")
        with col_larg: largeur_px=st.number_input("Largeur (px)",min_value=300,max_value=2000,value=900,step=50,key="iframe_largeur")
        hauteur_px=st.number_input("Hauteur (px)",min_value=400,max_value=5000,value=1800,step=100,key="iframe_hauteur")
        if position=="Pleine largeur": width_css="100%"; wrapper_css=""
        elif position=="Centré (milieu)": width_css=f"{largeur_px}px"; wrapper_css="text-align:center;"
        elif position=="Gauche": width_css=f"{largeur_px}px"; wrapper_css="text-align:left;"
        else: width_css=f"{largeur_px}px"; wrapper_css="text-align:right;"
        st.markdown("---")
        st.markdown('<div style="font-family:Space Mono;font-size:12px;color:var(--blue);margin-bottom:6px;">▶ Méthode 1 — iframe auto-hauteur</div>', unsafe_allow_html=True)
        ...
        st.markdown('<div style="font-family:Space Mono;font-size:12px;color:var(--blue);margin:16px 0 6px 0;">▶ Méthode 2 — iframe hauteur fixe</div>', unsafe_allow_html=True)
        st.code(f'<div style="{wrapper_css}">\n<iframe src="{url_veille}?v=TIMESTAMP"\n  style="width:{width_css}; height:{hauteur_px}px; border:none;" loading="lazy">\n</iframe>\n</div>', language="html")
        st.markdown('<div style="font-family:Space Mono;font-size:12px;color:var(--blue);margin:16px 0 6px 0;">▶ Méthode 3 — lien direct</div>', unsafe_allow_html=True)
        ...
        st.markdown("---")
        st.markdown('<div class="card card-accent" style="padding:14px 16px;"><div style="font-size:13px;font-weight:600;color:var(--blue);margin-bottom:8px;">📝 Note WordPress</div><div style="font-size:12px;color:var(--subtext);line-height:1.8;">Utilisez l\'éditeur en mode <strong style="color:var(--text)">HTML / Code source</strong>, le plugin <strong style="color:var(--text)">WPCode</strong>, ou un bloc <strong style="color:var(--text)">HTML personnalisé</strong>.</div></div>', unsafe_allow_html=True)
    st.markdown("---")
    c1,c2=st.columns(2)
    with c1:
    ...
    with c2:
        h=_historique(); nb_s=sum(1 for k in h if not k.startswith("__")); nb_a=sum(len(s.get("articles",[])) for ss in h.values() if isinstance(ss,list) for s in ss if isinstance(s,dict))
...
# ============================================================
# PAGES CONDITIONS + CONFORMITÉ
# ============================================================
def page_conditions():
    st.markdown("# 📄 Conditions d'utilisation"); st.markdown("---")
    st.markdown("""En utilisant Veille IA, vous acceptez que :\n- Le service collecte les données nécessaires au fonctionnement.\n- Les contenus analysés peuvent être traités par des services tiers.\n- Vous restez responsable des publications effectuées.\n- Le service est fourni "en l'état" et peut évoluer.\n- Vous pouvez demander la suppression de vos données.""")
    uid=_user_id()
    ...
    if st.button("✅ Accepter et continuer",type="primary",disabled=not cgu_ok):
        if not AUTH_OK or not uid: st.error("Session invalide."); return
        if hasattr(auth,"accepter_conditions"):
            res=auth.accepter_conditions(uid)
            if res.get("ok"): st.session_state["profil"]=auth.get_profil(uid); st.success("Conditions enregistrées."); _goto("veille")
            else: st.error(res.get("message","Erreur."))
        else: _goto("veille")
...
def page_conformite():
    ...
    for titre,contenu in [
        ("1) Responsable","**Editeur** : lucas rajany · **Contact** : lucas.rajanysio@gmail.com"),
        ("2) Données collectées","Email, configuration, historique de veille, journaux techniques."),
        ("3) Finalités","Fournir le service, générer des résumés IA, envoyer des emails si activé."),
        ("4) Base légale","Consentement (inscription + CGU) · Intérêt légitime (sécurité)."),
        ("5) Durée de conservation","Compte : durée d'utilisation · Historique : jusqu'à suppression par l'utilisateur."),
        ("6) Sous-traitants","Supabase (EU Frankfurt) · Groq · Gmail SMTP · Stripe"),
        ("7) Droits","Accès, rectification, effacement, portabilité via : lucas.rajanysio@gmail.com"),
        ("8) Sécurité","Contrôle d'accès, stockage segmenté par utilisateur, chiffrement en transit."),
    ]:
        with st.expander(titre): st.markdown(contenu)
    st.markdown("---")
