...
def generer_email_html(sujets_str: str, articles: list,
    ...
    if intervalle == 1:
        ...
        freq_label = f"tous les {intervalle} jours"
...
<body style='font-family:Arial,sans-serif;max-width:680px;margin:0 auto;color:#333;background:#f9f9f9;'>
    ...
    <div style='background:white;padding:24px 28px;'>
        ...
        <h2 style='color:#1565c0;border-bottom:2px solid #e3f2fd;padding-bottom:8px;font-size:16px;margin-top:28px;'>
    ...
    <div style='background:#e3f2fd;padding:14px 28px;border-radius:0 0 8px 8px;font-size:11px;color:#666;text-align:center;'>
        Veille {freq_label} — {date} · Pour modifier : onglet Automatisation dans Veille IA
    </div>
</body></html>"""
...
def traiter_utilisateur(user_id: str, sujets_str: str,
        ...
        try:
            ...
            if cfg.get("wp_base") and cfg.get("wp_user") and cfg.get("wp_password"):
                try:
                    ...
                    contenu  = srv.generer_contenu_html(historique, date_maj)
                    page_id  = srv.obtenir_ou_creer_page()
                    ok_wp, msg_wp = srv.publier_wordpress(contenu, page_id)
                    print(f"      WP  : {'✓' if ok_wp else '✗'} {msg_wp}")
                except Exception as e:
                    print(f"      WP  : ✗ {e}")
    ...
    html       = generer_email_html(sujets_str, articles_email, resume_all, intervalle)
...
def main():
    ...
    for u in utilisateurs:
        ...
        intervalle = int(u.get("intervalle_jours") or 1)
        email      = storage.get_user_email(user_id)

        if not email:
            print(f"  ✗ Email introuvable pour {user_id[:8]}… — ignoré"); continue
        if not sujets:
            print(f"  ✗ Aucun sujet pour {email} — ignoré"); continue

        traiter_utilisateur(user_id, sujets, email, intervalle)
