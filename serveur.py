...
try:
    ...
    _TIMEOUT_COURT    = getattr(_settings, "TIMEOUT_HTTP_COURT",         8)
    _TIMEOUT_STANDARD = getattr(_settings, "TIMEOUT_HTTP_STANDARD",     15)
    _TIMEOUT_LONG     = getattr(_settings, "TIMEOUT_HTTP_LONG",         30)
    _SCORE_MIN_PRI    = getattr(_settings, "SCORE_MIN_PRIMAIRE",        50)
    _SCORE_MIN_SEC    = getattr(_settings, "SCORE_MIN_SECONDAIRE",      35)
    _MAX_ARTICLES     = getattr(_settings, "MAX_ARTICLES_PAR_RECHERCHE",50)
    _DELAI_RESUME     = getattr(_settings, "DELAI_RESUME_SECONDES",      4)
    _DELAI_SYNTHESE   = getattr(_settings, "DELAI_SYNTHESE_SECONDES",   10)
except ImportError:
    _TIMEOUT_COURT    = 8
...
# ============================================================
# JSON EMBARQUÉ (import/export HTML)
# ============================================================
...
def est_domaine_bloque(domain: str) -> bool:
    ...
    if domain.endswith(".gouv.fr"):
        return True
    return any(d in domain for d in DOMAINES_GOUVERNEMENTAUX_BLOQUES)
...
def tester_connexion_wp(url, user, pwd):
    ...
    try:
        ...
        return False, {401:"Identifiants incorrects.",403:"Acces refuse.",404:"URL introuvable."}.get(r.status_code, f"Erreur {r.status_code}.")
    except requests.exceptions.ConnectionError:
        return False, "Site local inaccessible." if any(x in url.lower() for x in ["localhost",".local","127.0.0.1"]) else "Site distant inaccessible."
...
def _get_wp_headers():
    config = charger_config()
    token = base64.b64encode(f"{config.get('wp_user','')}:{config.get('wp_password','')}".encode()).decode("utf-8")
...
def obtenir_ou_creer_page():
    ...
    try:
        ...
        r = requests.post(f"{base}/pages", headers=_get_wp_headers(), json={
            "title":"Veille Technologique","content":"<p>Les résultats apparaîtront ici.</p>",
...
def est_bloque(r, sujet=""):
    ...
    if est_domaine_bloque(dom): return True
    if any(p in normaliser(url) for p in PATTERNS_GOUVERNEMENTAUX_BLOQUES): return True
    if any(t in titre for t in TITRES_HORS_SUJET): return True
    if sujet:
        mots = [m for m in re.split(r"[,\s]+", normaliser(sujet)) if len(m) > 2]
        if mots and not any(m in titre + " " + body for m in mots): return True
    return False
...
class ExtracteurTexte(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag.lower() in self.BALISES_IGNOREES: self._ignorer += 1
    def handle_endtag(self, tag):
        if tag.lower() in self.BALISES_IGNOREES: self._ignorer = max(0, self._ignorer - 1)
    def handle_data(self, data):
        if self._ignorer == 0:
            t = data.strip()
            if len(t) > 15: self.texte.append(t)
    def get_texte(self):
...
def extraire_texte_page(url):
    ...
    try:
        ...
        if "<html" not in html.lower(): return ""
        if html.count('\ufffd') > len(html) * 0.05: return ""
        p = ExtracteurTexte()
        p.feed(html)
...
def scorer_resultat(r, sujet):
    score = 0
    url   = r.get("href",""); titre = r.get("title",""); body = r.get("body","")
    dom   = urlparse(url).netloc.lower()
    tn    = normaliser(titre); bn = normaliser(body)
    ...
    if any(m in tn for m in ["dictionnaire","definition","lexique","glossaire","encyclopedie"]): score -= 100

    mots_fr = ["les","des","est","aux","que","qui","sur","pour","avec","dans","une","du","au","par","son","ses","ils","elle","nous","vous"]
    mots_en = ["the","of","and","to","in","is","that","was","for","are","with","they","this","from","have","been","will","not","but"]
    mots_autres = [" el "," los "," las "," del "," con "," por "," para "," como "," este "," esta "," pero "," sin "," sobre "," em "," no "," se "," ao "," da "," um "," sua "," il "," di "," che "," non "," per "," degli "]
    td = " " + tn + " " + bn[:300] + " "
    sf = sum(1 for m in mots_fr if f" {m} " in td)
    se = sum(1 for m in mots_en if f" {m} " in td)
    sa = sum(1 for m in mots_autres if m in td)
    est_fr = sf >= 2; est_en = se >= 2
    if sa >= 3 and not est_fr and not est_en: score -= 200
    ...
    for m in ["intelligence artificielle","machine learning","deep learning","ia generative","large language model","chatgpt","algorithme","automatisation","deepfake","biais algorithme","cybersecurite","attaque adversariale","menace numerique","vulnerabilite ia","securite des modeles","prompt injection","desinformation","bot malveillant","vie privee","rgpd","ethique ia","regulation ia","ai act europe","conformite ia","iot","objets connectes","internet des objets","reseau neuronal","traitement langage naturel"]:
        if m in tn: score += 15
        if m in bn: score += 5
...
def resumer_article_ollama(titre, url, body_ddg):
    ...
    try:
        resp = requests.post(
            ...
            json={"model":"llama-3.1-8b-instant","max_tokens":300,"messages":[
                {"role":"system","content":"Tu es un assistant de veille technologique. Tu résumes des articles en 3 à 5 points clés, en français, de manière concrète et factuelle."},
                {"role":"user","content":f"Article : \"{titre}\"\n\nContenu :\n{source[:2500]}\n\nRédige 3 à 5 points clés. Chaque point commence par \"• \". Phrases complètes. Uniquement les points."}
            ]}, timeout=_TIMEOUT_LONG)
        if resp.status_code == 200:
            contenu = resp.json()["choices"][0]["message"]["content"].strip()
            points  = [l.lstrip("•-–* ").strip() for l in contenu.splitlines()
                       if l.strip() and l.strip()[0] in "•-–*" and len(l.strip()) > 30]
            if points: return points[:5]
            phrases = [p.strip() for p in re.split(r'(?<=[.!?])\s+', contenu) if len(p.strip()) > 40]
...
def _resumer_fallback(source):
    if not source: return ["Résumé non disponible."]
    phrases = [p.strip() for p in re.split(r'(?<=[.!?])\s+', source) if 50 < len(p.strip()) < 400]
...
def generer_resume_global(sujet, articles):
    if not articles: return "Aucun article analysé."
    if not GROQ_API_KEY: return "Clé API Groq manquante (variable GROQ_API_KEY)."
    contexte = ""
    for i, a in enumerate(articles[:12]):
        titre  = a.get("title",""); dom = urlparse(a.get("href","")).netloc
        points = a.get("resume_ollama",[])
        inacc  = not points or points in [["Contenu non accessible pour ce site."],["Résumé non disponible."]]
        if not inacc:
            contexte += f"\n[{i+1}] {titre} ({dom})\n"
            for p in points[:3]: contexte += f"  - {p}\n"
        else:
            body = a.get("body","").strip()
            if body and len(body) > 40: contexte += f"\n[{i+1}] {titre} ({dom})\n  - {body[:300]}\n"
    if not contexte.strip(): return "Impossible de générer un résumé global."
    try:
        resp = requests.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"},
            json={"model":"llama-3.3-70b-versatile","max_tokens":3000,"messages":[
                {"role":"system","content":"Tu es un analyste en veille technologique. Tu rédiges des synthèses concrètes en français. JAMAIS de participe présent (-ant). JAMAIS de généralités. Quand tu utilises une info d'un article, ajoute [N] à la fin."},
                {"role":"user","content":f"""Voici les résumés de {len(articles)} articles sur : "{sujet}"\n\n{contexte}\n\nRÈGLES : cite chaque source {", ".join(f"[{i+1}]" for i in range(min(len(articles),12)))} au moins une fois.\n\nFormat :\n— Menaces & risques identifiés\n- [fait] [N]\n\n— Comment se protéger\n- [conseil] [N]\n\n— Outils & technologies\n- [outil] [N]\n\n— Tendances à surveiller\n- [tendance] [N]\n\nPhrases directes. N'invente rien."""}
            ]}, timeout=_TIMEOUT_LONG)
        if resp.status_code == 200:
            ...
            resp2 = requests.post(
                ...
                json={"model":"llama-3.3-70b-versatile","max_tokens":3000,
                      "messages":[{"role":"user","content":contexte[:2000]}]}, timeout=_TIMEOUT_LONG)
            if resp2.status_code == 200: return resp2.json()["choices"][0]["message"]["content"].strip()
...
def charger_historique():   return _charger_historique_ctx()
def sauvegarder_historique(h): _sauvegarder_historique_ctx(h)
def effacer_historique():   _sauvegarder_historique_ctx({})

def comparer_sessions(sujet, session_nouvelle, session_ancienne):
    date_new = session_nouvelle.get("date","récente"); date_old = session_ancienne.get("date","précédente")
    ...
    if not nouveaux and not disparus: return f"Aucune différence entre les sessions du {date_old} et du {date_new}."
    ...
    if nouveaux:
        ...
        for a in nouveaux[:6]:
            ...
            if pts and pts != ["Contenu non accessible pour ce site."]:
                for p in pts[:2]: contexte += f"  · {p}\n"
    if disparus:
        contexte += f"\nARTICLES DISPARUS ({len(disparus)}) :\n"
        for a in disparus[:4]: contexte += f"- {a.get('title','')}\n"
    if not GROQ_API_KEY: return contexte
    try:
        resp = requests.post(
            "https://api.groq.com/openai/v1/chat/completions",
            headers={"Authorization": f"Bearer {GROQ_API_KEY}", "Content-Type": "application/json"},
...
def rechercher(sujet_brut, callback_statut=None):
    ...
    try:
        with DDGS() as ddgs:
            for ss in sous_sujets:
                for q in [ss+" intelligence artificielle actualite", ss+" IA "+str(datetime.now().year), ss+" artificial intelligence"]:
                    for r in (ddgs.text(q, region="fr-fr", max_results=25) or []):
                        lien = r.get("href","#")
                        if lien not in ids_vus:
                            ids_vus.add(lien); r["sous_sujet"] = ss; data.append(r)
    except Exception as e:
        ...
        for url_flux in FLUX_RSS_IA:
            try:
                ...
                for entry in feed.entries[:20]:
                    lien = entry.get("link",""); titre = entry.get("title",""); body = entry.get("summary","")
                    if not lien or lien in ids_vus: continue
                    if any(m in normaliser(titre) or m in normaliser(body) for m in mots):
                        ids_vus.add(lien); data.append({"title":titre,"body":body,"href":lien,"source":"rss"})
            except Exception:
                continue
    ...
    if not filtres: filtres = [r for r in data if r["score"] >= _SCORE_MIN_SEC]
...
def _formater_resume_html(texte, articles_ref):
    ...
    def remplacer_citation(m):
        ...
        if num in index_urls:
            ...
            return f' <a href="{url_r}" target="_blank" style="color:var(--blue);font-size:11px;text-decoration:none;" title="{dom_r}">[{num}]</a>'
        return m.group(0)
    ...
    for i, session in enumerate(sessions):
        ...
        <div class="session-block">
            <button class="session-header" onclick="toggleSession('{sid}')">
                ...
                <span id="arr-{sid}" class="sess-arrow">{"▲" if is_open else "▼"}</span>
            </button>
            <div id="{sid}" style="display:{"block" if is_open else "none"}">"""

        if rg and not rg.startswith("Erreur"):
            html += f"""
                <div class="synth-box">
                    <div class="synth-title">Synthèse — {date_s}</div>
                    <div class="synth-body">{_formater_resume_html(rg, articles)}</div>
                </div>"""
        ...
        for a in articles:
            ...
            if pts and pts not in [["Contenu non accessible pour ce site."],["Résumé non disponible."]]:
                pts_html = f"<ul class='art-pts'>{''.join(f'<li>{p}</li>' for p in pts[:3])}</ul>"
            html += f"""
                <div class="art-card">
                    <a href="{a.get('href','#')}" target="_blank" class="art-title">{a.get('title','')}</a>
                    {"<span class='badge-doublon'>~ doublon</span>" if doublon else ""}
                    <div class="art-meta">
...
def generer_contenu_html(historique, date):
    ...
    sujets = [k for k, v in historique.items() if not k.startswith("__") and isinstance(v, list) and v]
    if not sujets:
        return "<p style='color:var(--sub);font-style:italic;padding:20px;'>Aucun résultat.</p>"

    html = f"<span class='maj-date'>Dernière mise à jour : {date}</span><hr>"
    for i, sujet in enumerate(sujets):
        ...
        <div class="sujet-block">
            <button class="sujet-header" onclick="toggleSujet('{sid}')">
                ...
                <span id="sujet-arrow-{sid}" class="sujet-arrow">{"▲" if is_open else "▼"}</span>
            </button>
            <div id="sujet-body-{sid}" class="sujet-body" style="display:{"block" if is_open else "none"}">
...
def generer_html_complet(contenu_body, date, historique_embed=None):
    theme_vars = "--bg:#1e1e2e;--surf:#181825;--ov:#313244;--txt:#cdd6f4;--sub:#a6adc8;--brd:#45475a;--blue:#89b4fa;--grn:#a6e3a1;--yel:#f9e2af;--red:#f38ba8;"
...
def generer_html_complet_theme(contenu_body: str, date: str, theme: dict, historique_embed=None) -> str:
    bg=theme.get("bg","#1e1e2e"); surf=theme.get("surf","#181825"); ov=theme.get("ov","#313244")
    txt=theme.get("txt","#cdd6f4"); sub=theme.get("sub","#a6adc8"); brd=theme.get("brd","#45475a")
    blue=theme.get("blue","#89b4fa"); grn=theme.get("grn","#a6e3a1"); yel=theme.get("yel","#f9e2af")
    red=theme.get("red","#f38ba8"); font=theme.get("font","Arial,sans-serif")
    fs=theme.get("fs","13"); ptitle=theme.get("ptitle","Veille Technologique IA")
    theme_vars = f"--bg:{bg};--surf:{surf};--ov:{ov};--txt:{txt};--sub:{sub};--brd:{brd};--blue:{blue};--grn:{grn};--yel:{yel};--red:{red};"
    embed = _format_embedded_historique(historique_embed) if historique_embed else ""
...
def publier_wordpress(contenu, page_id):
    base = _wp_base()
    if not base or not page_id: return False, "Config WordPress manquante"
    try:
        r = requests.post(f"{base}/pages/{page_id}", headers=_get_wp_headers(),
                          json={"title":"Veille Technologique","content":contenu,"status":"publish"},
                          timeout=_TIMEOUT_STANDARD)
        return (True,"WordPress mis a jour") if r.status_code in [200,201] else (False,f"Erreur WP {r.status_code}")
    except Exception as e:
...
def _uploader_ftp(html_final: str, chemin: str = None) -> tuple:
    ...
    try:
        ...
        if not chemin:
            racine  = ftp.nlst(); dossier = "/htdocs"
...
def creer_post_wordpress(contenu, sujet, date):
    base = _wp_base()
    if not base: return False, "Config WordPress manquante"
    try:
        r = requests.post(f"{base}/posts", headers=_get_wp_headers(),
                          json={"title":f"{sujet} — {date}","content":contenu,"status":"publish"},
                          timeout=_TIMEOUT_STANDARD)
        return (True,"Post cree") if r.status_code in [200,201] else (False,f"Erreur WP {r.status_code}")
    except Exception as e:
...
def supprimer_anciens_posts():
    ...
    try:
        ...
        if not isinstance(posts, list): return 0
        supprimes = 0
        for post in posts:
...
def workflow_publier(sujet, resultats_recherche, callback_statut=None,
    ...
    for r in resultats_recherche:
        href = r.get("href","")
        if not href or href in tous_hrefs: continue
        if len(nouveaux_articles) >= nb_max: break
        ...
        nouveaux_articles.append({
            "title":r.get("title",""),"href":href,"score":r.get("score",0),
